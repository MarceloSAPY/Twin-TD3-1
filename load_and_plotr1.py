...
def read_init_location(entity_type = 'user', index = 0):
    if entity_type == 'user' or 'RIS' or 'RIS_norm_vec' or 'UAV':
        return np.array([\
...
class LoadAndPlot(object):
    ...
    def plot(self):
        ...
        ###############################
        # plot average sum user capacity of each episode
        ###############################
        fig = plt.figure('average_sum_user_capacity')
        sum_user_capacity = np.array(self.all_steps['user_capacity'])
        sum_user_capacity = np.sum(sum_user_capacity, axis = 0)
        average_sum_user_capacity = []
        j = 0
        for i in range(self.ep_num):
            uc_one_episode = sum_user_capacity[j:j+step_num_per_episode[i]] # uc means User Capacity
            j = j+step_num_per_episode[i]
            try:
                _ = sum(uc_one_episode) / len(uc_one_episode)
            except:
                _ = 0
            average_sum_user_capacity.append(_)
        plt.plot(range(len(average_sum_user_capacity)), average_sum_user_capacity)
        plt.xlabel("Episodes (Ep)")
        plt.ylabel("Average Sum User Capacity")
        plt.savefig(self.store_path + 'plot/average_sum_user_capacity.png')
        plt.cla()
        ...
        print('###########################################################')
        print('Sum User Capacity\t\t{:.2f}\t\t{:.2f}'.format(average_sum_user_capacity[-1], max(average_sum_user_capacity)))
