...
parser.add_argument('--uav_ant_num', type=int, default=2, help="Number of UAV antennas (Nguyen uses 2)") # NUEVO
...
STEP_NUM = args.step_num # CORREGIDO: Usar el argumento
UAV_ANT_NUM = args.uav_ant_num # CORREGIDO: Usar el argumento
...
# Nombre del proyecto para guardar datos
project_name = f'trained_uav/{DRL_ALGO}_{REWARD_DESIGN}' if TRAINED_UAV else f'scratch/{DRL_ALGO}_{REWARD_DESIGN}'

system = MiniSystem(
    user_num=4,
    RIS_ant_num=32,
    UAV_ant_num=UAV_ANT_NUM, # CORREGIDO: Usa 2 si no se especifica lo contrario
    if_dir_link=1,
    if_with_RIS=True,
    if_move_users=False,
    if_movements=True,
...
agent_1_param_dic["batch_size"] = 256
# Corrección: system.get_system_action_dim() ya devuelve 2 (UAV) + 32 (RIS) = 34
# Restamos 2 porque este agente SOLO controla el RIS (32)
...
agent_2_param_dic["input_dims"] = 3 + system.user_num # Posición + Capacidades
agent_2_param_dic["tau"] = 0.001
agent_2_param_dic["batch_size"] = 256
...
if TRAINED_UAV:
    ...
    # Cargar modelos... (Código existente)

# Guardar metadatos (Código existente...)
# ...
    ...
    while step_cnt < step_num:
        ...
        if not system.render_obj.pause:
            ...
            if system.if_with_RIS:
                # --- CORRECCIÓN CRÍTICA PARA ENV MRT ---
                # Pasamos action_1 directo a Phi. G se pasa vacío (calculado por MRT).
                new_state_1, reward, done, info = system.step(
                    action_0=action_2[0],
                    action_1=action_2[1],
                    G=[],            # Ignorado por env (usa MRT)
                    Phi=action_1,    # Agente 1 controla solo el RIS
